...
- **Einsum `contraction_steps`:** **non-final** steps use the **immediate** operand footprint (tensors not in that `einsum` are excluded); the **last** step uses **full lineage** for a global box. Nested padding bonus still separates nested boxes.
- **Contraction scheme:** default **`PlotConfig.contraction_scheme_alpha`** is **1.0** (opaque 2D fills); set a lower value for translucent highlights.
