...
def _theta_classes_2d(nx_graph: nx.Graph) -> tuple[_ThetaClass2D, ...] | None:
    ...
    edge_ids = tuple(
        tuple(sorted((int(left_id), int(right_id)))) for left_id, right_id in nx_graph.edges()
    )
    parent: dict[tuple[int, int], tuple[int, int]] = {edge_id: edge_id for edge_id in edge_ids}

    def find(edge_id: tuple[int, int]) -> tuple[int, int]:
        while parent[edge_id] != edge_id:
            parent[edge_id] = parent[parent[edge_id]]
            edge_id = parent[edge_id]
        return edge_id

    def union(left_edge: tuple[int, int], right_edge: tuple[int, int]) -> None:
    ...
    grouped_edges: dict[tuple[int, int], list[tuple[int, int]]] = {}
